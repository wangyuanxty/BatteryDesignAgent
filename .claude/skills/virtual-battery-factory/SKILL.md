...
7. **设计交付物（行业标准文件）**：收尾必须产出一份 **电芯设计规格书 `design_spec.md`**——电池设计领域的标准交付文档，字段全部机械取自参数集与仿真结果（不得凭记忆写数）：电化学体系、标称容量、电压窗口、电极/隔膜/集流体参数（厚度/孔隙率/N/P 比）、电解液配方、**工艺设计参数**（面密度 = 厚度×(1−孔隙率)×电极密度；压实密度 = 电极密度×(1−孔隙率)，kg/m³→g/cm³ 除 1000；注液量 = 孔隙体积×电解液密度（文献值标注）×注液系数；化成建议值标注"设计建议，产线实值需调试"）、质量明细（按合同公式）、性能验证表（容量/能量密度/温升/析锂，附达标判定）、设计说明（改了哪些参数、为什么）。数据来源逐行标注（参数集/仿真结果/文献），缺失项如实写"未提供"。
