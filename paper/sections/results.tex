...
because they are the single criterion class whose parameter bridge (the
...
material-bottleneck tasks the optimizer's failure is bound by reachability, not
...
plating and SEI violations are bound by the available variables (supplementary materials). On the
...
T5 flagship & $\checkmark$ & $\checkmark$ & $\times$ (cannot reach) & $\checkmark$ 517.8 & $\times$ ($-12.6$ mV) & $\times$ (plating) \\
...
and the decomposition is the paper's answer to RQ2. Reachability escalation is the load-bearing
...
Architecture forcing matters narrowly: disabling it changes exactly one
...
segments count the unchanged tasks. Reachability escalation is causal for two of
...
\begin{tabular}{lcccc}
\toprule
Task & reachability off & force off & voting off & full workflow \\
\midrule
...
governed DeepSeek runs needed the reachability-escalation mechanism: T1 falls to
...
reach the material-level variable, so the honest result is a lower score rather than
...
\textbf{Pre-registration, reachability check, iteration.} The agent's
...
The baseline characterization then records the reachability check in
action: ``R1 --- baseline characterization (reachability check): Chen2020
...
R02 & Gap quantification on the LNMO design: plating and SEI $>$500 nm; SEI growth is temperature-insensitive, so cooling is not the variable. & r2\_lnmo\_aging; r2\_lnmo\_4c (h=200/500) & plated; SEI 753.1 & $\times\times\times$ \\
...
R05 & Combine the measured variables into one shipping candidate; run the complete SPMe evidence battery. & r5\_final\_\_spme (1c/aging/4c) & 1135.2 Wh/L; 4.188 V; 321.09 K; SEI 435.4 & $\checkmark$ (SPMe) \\
...
All non-molecular-level variables are prohibited in all conditions (the
...
knobs and no molecular-level variable. Its best design after 50 trials reaches
energy density $678.7$ Wh/kg---the highest in any condition, because
structural variables are the only ones BO has---but its aging result is
...
fixed rule and replayed to the digit, while BO has no material-level variable to
