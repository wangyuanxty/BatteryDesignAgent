...
### 5b. 和厂商规格书比之前，必须处理质量口径（2026-09-26）
...
**附一条：验证清单已经查实，见 `battery-design-map.md` 与本文第二节。** PyBaMM 自带 **26 个验证数据文件**，其中**是实测**的是三组：Kokam SLPB75106100（1C/5C）、Enertech（0.1C/0.5C/1C/2C ＋ 温度 ＋ **位移**）、行驶循环（UDDS/US06/WLTC）。**COMSOL 那 6 个是软件对软件，不是实测，不能当锚。**
