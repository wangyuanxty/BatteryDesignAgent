...
| **③ 仿真器签字** | 证明它算出来的数**不是自说自话** | **最弱的一环**，见第五节。现有 4 条锚（LG M50 / Kokam / Enertech / 行驶循环），**但低温、高温老化、快充、针刺一条都没有** |
...
| **能量密度 Wh/kg、Wh/L** | ✅ 但**口径要当心** | 我们是电极堆口径、规格书是整芯口径，**差约 1.6 倍**（见 5b）。比之前必须处理 |
...
| **日历老化** | ❌ | **没有静置协议**（`PROTOCOLS` 8 个里没有）＋ `SEI growth activation energy` 在 Chen2020 = 0 → **温度完全不生效** |
...
2. **⚠️ 的可以进，但要写清条件**：倍率必须 DFN；循环寿命必须换判据；快充要与厂商口径对齐。
3. **✅ 的可以进，是任务的主力。**
...
### 5a. 锚的清单（2026-09-26 查实）

**一个电芯能当锚，要同时有 ① 参数集、② 与该参数集同款的实测。** 两者齐的现在有这些：

| 锚 | 参数集 | 实测（来源） | 覆盖工况 | 特别之处 |
|---|---|---|---|---|
| **LG M50**（21700，NMC811 ＋ 石墨-SiOx） | Chen2020 / ORegan2022 / OKane2022 等 **6 套** | Imperial College 数据集（**我们自己接的**）<br>Zenodo 10.5281/zenodo.10637534，CC BY 4.0；论文 Kirkaldy, N.; Samieian, M. A.; Offer, G. J.; Marinescu, M.; Patel, Y. *J. Power Sources* **2024**. DOI: 10.1016/j.jpowsour.2024.234185 | **0.1C 放电**，10/25/40 °C 多 SOC 区间 | 锚定结果 **0.45% 容量 / 20.6 mV RMSE**；和我们大部分任务同体系 |
| **Kokam SLPB75106100**（软包，NCO/石墨） | Ecker2015 ×2 | **PyBaMM 自带**（`Ecker_1C.csv`、`Ecker_5C.csv`） | **1C、5C** | **倍率锚** |
| **Enertech**（软包，2.28 Ah） | Ai2020 | **PyBaMM 自带**（13 个文件） | **0.1C / 0.5C / 1C / 2C**，且同时有**电压 ＋ 温度 ＋ 位移** | **唯一带完整裂纹参数**（Ai2020），**且自带形变实测**——唯一能验开裂模型的锚 |
| **行驶循环**（不是一个电芯） | —— | **PyBaMM 自带**（`UDDS.csv`、`US06.csv`、`WLTC.csv`、`car_current.csv`） | 动态工况 | **动态工况锚** |

**⚠️ 不要搞错的一条**：PyBaMM 自带的 **26 个验证文件**里，**COMSOL 那 6 个（0.1C–3C、1+1D-3C）是软件对软件**（PyBaMM vs COMSOL），**不是实测，不能当锚**。

**有参数集但没有配套实测的，不能当锚**：Mohtat2020（NMC532 软包）、NCA_Kim2011（建模论文的设计算例，**不是实测商品电芯**）、Marquis2019 / Xu2019（归属存疑——两套写同一个 Kokam 型号但几何差一个量级；Xu2019 的 docstring 说钴酸锂、代码赋的是 NMC）。

### 5b. 还缺什么

**我们的任务涉及 5C 倍率、−20 °C 低温、4C 快充、45 °C 老化、针刺**——现有四条锚里：
