"""Run the 4-sim chain for one round-4 candidate (subprocess, absolute paths)."""
import subprocess
import sys
...
name = sys.argv[1]
tag = name.replace(" ", "_")
params = CELL / f"params_r4_{tag}.json"

steps = [
    (["run-pyamm", "--params", str(params), "--protocol", "5C_discharge", "--mode", "dfn",
      "--thermal", "lumped", "--plating", "--out", str(CELL / f"r4_{tag}_5c_dfn.json")],
     "5c"),
    (["run-pyamm", "--params", str(params), "--protocol", "4C_charge_45C", "--mode", "dfn",
      "--thermal", "lumped", "--plating", "--out", str(CELL / f"r4_{tag}_4c_safety.json")],
     "4c"),
    (["run-pyamm", "--params", str(params), "--protocol", "1C_discharge", "--mode", "dfn",
      "--thermal", "lumped", "--plating", "--out", str(CELL / f"r4_{tag}_1c_dfn.json")],
     "1c"),
    (["calc-energy", "--sim", str(CELL / f"r4_{tag}_1c_dfn.json"), "--params", str(params),
      "--out", str(CELL / f"r4_{tag}_energy.json")],
     "energy"),
]
