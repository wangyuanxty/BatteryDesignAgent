"""t4_r3: derive round-5 retentions + summarize V10-V13."""
import json
...
for v in ("V10", "V11", "V12", "V13"):
    lowt = json.loads((ws.path / f"cell/r5_{v}_1c_lowT_spme.json").read_text(encoding="utf-8-sig"))
    rt = json.loads((ws.path / f"cell/r5_{v}_1c_25C_spme.json").read_text(encoding="utf-8-sig"))
    en = json.loads((ws.path / f"cell/r5_{v}_energy.json").read_text(encoding="utf-8-sig"))
    hot = json.loads((ws.path / f"cell/r5_{v}_4C_45C_spme.json").read_text(encoding="utf-8-sig"))
    ...
    (ws.path / f"bridge/r5_{v}_lowT_retention.json").write_text(
        json.dumps(out, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    print(
        f"r5_{v}: retention={pct:6.2f}%  cap25C={rt['capacity_ah']:.4f}  cap_lowT={lowt['capacity_ah']:.4f}  "
