"""Driver: run D1/D2 energy+aging and full E1/E2 sim sets. Sequential, prints per step."""
import subprocess
import sys
...
steps = []
# D1/D2: energy calc from existing 1C sims + aging (needed for the round-5 evaluate)
for tag in ("D1", "D2"):
    params = str(CELL / f"r5_{tag}_params.json")
    sim1c = str(CELL / f"r5_{tag}_1c_spme.json")
    steps.append((["calc-energy", "--sim", sim1c, "--params", params, "--base", BASE,
                   "--out", str(CELL / f"r5_{tag}_energy.json")], f"r5_{tag}_energy"))
    steps.append((["run-pyamm", "--params", params, "--protocol", "aging_1C_100cyc",
                   "--base", BASE, "--mode", "spme", "--thermal", "lumped", "--cycles", "100",
                   "--out", str(CELL / f"r5_{tag}_aging_spme.json")], f"r5_{tag}_aging"))
# E1/E2: full sets
for tag in ("E1", "E2"):
    params = str(CELL / f"r6_{tag}_params.json")
    steps.append((["run-pyamm", "--params", params, "--protocol", "1C_discharge",
                   "--base", BASE, "--mode", "spme", "--thermal", "lumped",
                   "--out", str(CELL / f"r6_{tag}_1c_spme.json")], f"r6_{tag}_1c"))
    steps.append((["calc-energy", "--sim", str(CELL / f"r6_{tag}_1c_spme.json"), "--params", params,
                   "--base", BASE, "--out", str(CELL / f"r6_{tag}_energy.json")], f"r6_{tag}_energy"))
    steps.append((["run-pyamm", "--params", params, "--protocol", "4C_charge_45C",
                   "--base", BASE, "--mode", "spme", "--thermal", "lumped", "--plating",
                   "--out", str(CELL / f"r6_{tag}_4c_spme.json")], f"r6_{tag}_4c"))
    steps.append((["run-pyamm", "--params", params, "--protocol", "aging_1C_100cyc",
                   "--base", BASE, "--mode", "spme", "--thermal", "lumped", "--cycles", "100",
                   "--out", str(CELL / f"r6_{tag}_aging_spme.json")], f"r6_{tag}_aging"))
